...
CREATE TABLE IF NOT EXISTS public.course_schedule_week
(
    id                 BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    mandant_id         BIGINT NOT NULL DEFAULT 7777,
...
CREATE TABLE IF NOT EXISTS public.course_course_level
(
    mandant_id      BIGINT NOT NULL DEFAULT 7777,
...
CREATE TABLE IF NOT EXISTS public.course_weekday
(
    mandant_id BIGINT NOT NULL DEFAULT 7777,
    course_id  BIGINT NOT NULL,
    sort_index INT    NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.participant_course
(
    mandant_id     BIGINT NOT NULL DEFAULT 7777,
...
CREATE TABLE IF NOT EXISTS public.participant_course_level
(
    mandant_id      BIGINT NOT NULL DEFAULT 7777,
    participant_id  BIGINT NOT NULL,
    course_level_id BIGINT NOT NULL,
...
DROP TABLE IF EXISTS public.mandant_settings CASCADE;

CREATE TABLE public.mandant_settings(
    id                                BIGINT PRIMARY KEY,
    ...
    is_parent_one_main_parent          BOOLEAN NOT NULL DEFAULT TRUE,
    course_report_name                 VARCHAR(255) NOT NULL DEFAULT 'course-report-',
    limit_course_assignments_to_active BOOLEAN NOT NULL DEFAULT FALSE,

    CONSTRAINT fk_mandant_settings_mandant
        FOREIGN KEY (id) REFERENCES public.mandants (id) ON DELETE RESTRICT
);

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'uk_mandant_settings_mandant') THEN
ALTER TABLE public.mandant_settings
    ADD CONSTRAINT uk_mandant_settings_mandant UNIQUE (id);
END IF;
END $$;
